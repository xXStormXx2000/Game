...
HexagonGrid::HexagonGrid(Vector3D origin, double size): origin(origin), size(size) {}
Vector3D HexagonGrid::getHexCordAtPoint(Vector3D point) {
	...
	double x_ = floor(i * point/this->size);
	double y_ = floor(j * point/this->size);
	double z_ = floor(k * point/this->size);

	double x = x_ + (int)(floor((y_ + z_ + 2)/3) + x_)%2;
	double z = z_ + (int)(floor((y_ + x_ + 2)/3) + z_)%2;
